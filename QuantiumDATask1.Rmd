...
library(arules)
library(arulesViz)
# Subset data to Mainstream, young singles/couples segment
main_single <- subset(df,PREMIUM_CUSTOMER == "Mainstream" & LIFESTAGE == "YOUNG SINGLES/COUPLES")
# Create transaction object
transB <- as(main_single$Brand_name, "transactions")
# Perform affinity analysis using Apriori algorithm
rules <- apriori(transB, parameter = list(supp = 0.001, conf = 0.8))
# Visualize the rules
...
# Subset data to Mainstream, young singles/couples segment
cust_seg <- subset(data, PREMIUM_CUSTOMER == "Mainstream" & LIFESTAGE == "YOUNG SINGLES/COUPLES")

# Create transaction object
trans <- as(cust_seg$Brand_name, "transactions")
